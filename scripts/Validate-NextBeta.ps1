...
$coreValidator = Join-Path $PSScriptRoot "Validate-NextBeta-Core.ps1"
...
if (-not (Test-Path -LiteralPath $coreValidator -PathType Leaf)) {
    throw "Internal combined validator core is missing: $coreValidator"
}
...
try {
    ...
    & $coreValidator -Pr $Pr -Repository $Repository -WerLookbackDays $WerLookbackDays -WorkingRoot $WorkingRoot
}
catch {
    $coreError = $_
}
